...
import { createBooking, delete_booking, get_booking, get_user_booking } from "@/apis/booking_apis";
        ...
        try {
            ...
            if (responseGetUserBook.status === 200) {
                ...
                set({
                    userBooking: [],
                    loading: false,
                    error: responseGetUserBook.data.message || 'Error fetching user bookings'
                });
            }
        } catch (error) {
            set({loading: false, error: (error as Error).message});
